...
export const GAMES = {
  ...
  mountains: {
    ...
    data: {
      ...
      select: 'id, short_name, elevation, description, countries, range, region',
    },
    mechanics: {
      ...
      filters: {
        ...
        // UN M49 sub-regions, same taxonomy as history (continent-grouped)
        group: { key: 'region', column: 'region', grouped: true },
      },
    },
  },
  ...
  rivers: {
    ...
    tagline: { en: 'Which river is longer?', cs: 'Která řeka je delší?', it: 'Quale fiume è più lungo?' },
    data: {
      table: 'rivers',
      translationsTable: 'river_translations',
      translationFkColumn: 'river_id',
      select: 'id, short_name, length, description, countries, region',
    },
    mechanics: {
      getValue: (item) => item.length ?? 0,
      getComparable: (item) => item.length ?? 0,
      direction: 'higher',
      minGap: 10,
      gapScale: 500,
      easyGap: 100,
      filters: {
        range: { minKey: 'minLength', maxKey: 'maxLength' },
        group: { key: 'region', column: 'region', grouped: true },
      },
    },
  },
